...
interface VerticalHeroProps {
  /** Eyebrow tag — vertical name in uppercase (e.g. "FACING FORECLOSURE") */
  eyebrow: string
  /** Headline before the green accent clause */
  headlineLead: string
  /** The clause rendered in green (e.g. "fast, no fees.") */
  headlineAccent: string
  /** Sub-copy paragraph */
  subheadline: string
  /** Used to set source_detail on the lead form */
  landingPage: string
  /** Vertical-specific testimonial overlaid on the photo card */
  testimonial: {
    quote: string
    authorName: string
    authorCity: string
  ...
  return (
    <section className="relative min-h-[700px] flex items-center overflow-hidden bg-brand-bg-warm">
      <div className="mx-auto max-w-7xl px-6 md:px-8 w-full grid md:grid-cols-2 gap-12 md:gap-16 items-center py-12 md:py-0">
        ...
        {/* RIGHT — photo card with vertical-specific testimonial overlay */}
        <div className="relative hidden md:block">
          <div className="aspect-[4/5] rounded-[2rem] overflow-hidden shadow-ambient">
            <Image
              src="/brand/hero-house.jpg"
              alt="Kansas City home — the kind of property Heartland buys for cash"
              width={800}
              height={1000}
              priority
